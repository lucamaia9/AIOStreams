...
**Fix — Two-part change following TRaSH Guides per-indexer proxy pattern:**

1. **`flaresolverr-prowlarr` routed through Amsterdam VPN:**
   - Added `HTTP_PROXY=http://adguard-proxy-amsterdam:8888` (mirrors existing `flaresolverr-helsinki` pattern)
   - Added `depends_on: adguard-proxy-amsterdam: condition: service_healthy`
   - All FlareSolverr CF-solve traffic now egresses through Amsterdam exit IP
   - Prowlarr itself remains direct egress (TRaSH Guides recommendation)

2. **Per-indexer HTTP proxy for slow indexers:**
   - Created tag `amsterdam-lane` (ID 9) and `Http-Amsterdam` IndexerProxy
   - Tagged Mikan (ID 42), The Pirate Bay (ID 26), Torrent9 (ID 28) with `amsterdam-lane`
   - These 3 keep their `flaresolver` tag (ID 1) for CF fallback through Amsterdam-backed FlareSolverr

**Files changed:** `compose.yaml`, `docs/INFRASTRUCTURE.md`, `docs/info.md`

## 2026-05-19 -- BitMagnet/MagneticoLocal Improvements

**Priority 1 — SQLite Index Cleanup:**
- Identified and dropped 5 duplicate indexes on `media_index` in `active.search.sqlite3`
- Duplicates were on columns: `content_class`, `is_anime`, `norm_title`, `resolution`, `(info_hash, content_class)`
- Each had an explicit index and an implicit one via column constraints
- Database backed up before change (1,004 MB `.bak`)
- Integrity verified: `PRAGMA quick_check` → ok, 12 indexes remaining

**Priority 2 — Anime Classification Fixes:**
- `ANIME_DASH_EPISODE_PATTERN` in `shared_episode_structure.py`, `compact_media_search.py`: now matches bracket-range episodes like `[07-13]` via lookbehind `(?<=\d)` and `]` terminator
- `ANIME_GROUP_PATTERN` in all 3 classifiers: added 7 real anime subgroups (`DMG&SumiSora`, `AnimeChap`, `LoliHouse`, `Ani-One`, `Nekosama-Subs`, `Lilith-Raws`, `SkyMoon-Raws`, `Orion-Origin`); fixed `anione` to `ani[ ._-]?one`
- `ANIME_HINT_PATTERN` in all 3 classifiers: removed `vostfr` (causes false positives on French media); added `bdmv`, `ova`, `ona` as reliable anime indicators
- All regex patterns verified with smoke tests — compiles and matches correctly

**Priority 3 — Fixed hardcoded Go path:**
- `internal/processor/config.go`: `ScriptPath` was pointing to `/home/ubuntu/bitmagnet-media/scripts/...` which doesn't exist — changed to correct path `bitmagnet-media/...` relative to project root
- Both `bitmagnet_live_promotion.sh` and `bitmagnet_backfill_and_prune.sh` were affected

**Priority 4 — Script path cleanup:**
- `scripts/bitmagnet/bitmagnet_import_watch.sh`: added header comment marking it stale after the `bitmagnet-media/` reorganization
- `scripts/bitmagnet/run_bitmagnet_import.sh`: same

## 2026-04-23 -- Root Directory Reorganization (Phase 3 — Aggressive Cleanup)

Root entries reduced from 30 files to 4 meaningful files + `docs/` directory. ~890 MB freed.

**Deleted (upstream build files not needed on VPS — recoverable from git):**
- `node_modules/` (889 MB), `Dockerfile`, `package.json`, `pnpm-lock.yaml`, `pnpm-workspace.yaml`, `tsconfig.base.json`, `tsconfig.json`, `.dockerignore`, `.prettierrc`, `release-please-config.json`, `.release-please-manifest.json`, `.env.sample`, `CHANGELOG.md`, `README.md`, `LICENSE`, `.vscode/`, `.github/`

**Moved to `docs/`:**
- `BITMAGNET_OPERATIONS.md`, `COMET_OPERATIONS.md`, `IPTV_OPERATIONS.md`, `INFRASTRUCTURE.md`, `PROJECT_OVERVIEW.md`, `info.md`

**Moved to scripts:**
- `SCRIPTS_CATALOG.md` → `scripts/SCRIPTS_CATALOG.md`
- `TESTING_RUNBOOK.md` → `scripts/testing/TESTING_RUNBOOK.md`

**Scripts reclassification (eliminated `misc/` catch-all):**
- `run_production_gates.py`, `live_enrichment_worker.py`, `batch_fuzzy_update.py` → `scripts/bitmagnet/`
- `generate_patterns_go.py`, `compare_go_vs_python_filter.py` → `scripts/testing/`
- `prepare_bitmagnet_live_indexes.sh` → `scripts/archived/` (legacy)

**Runtime cleanup:**
- Deleted `test_runs/`, 0-byte lock files, stale `jackett/config/`, orphaned `aiometadata/data/database.sqlite`
- Trimmed prowlarr logs (103 → 15 files, saved ~85 MB) and backups (kept latest 2, saved ~65 MB)
- Cleaned jackett `.bak` files and rotated logs

**Stale reference fixes (pre-existing bugs):**
- `bitmagnet-media/README.md`: broken `BITMAGNET_PIPELINE_ROADMAP.md` link
- `bitmagnet-media/docs/01-start-here/README.md`: same
- `bitmagnet-media/docs/03-operations/OPERATIONS.md`: wrong `ROLLOUT.md` path
- `BITMAGNET_OPERATIONS.md`: stale `docs/DOCUMENTATION_INDEX.md` reference
- `bitmagnet-media/docs/06-historical/TECHNICAL_DEEP_DIVE.md`: broken `BITMAGNET_TARGET_LOGIC.md`, non-existent `LIVE_ENRICHMENT_IMPLEMENTATION.md` and `PLAN.md`
- `TESTING_RUNBOOK.md`: invalid `python3 -m unittest` command with filesystem path

**All cross-references updated** across ~25 files. `docker compose config` validates cleanly.

## 2026-04-22 -- Root Directory Reorganization (Phase 2)

Consolidated BitMagnet-scoped and IPTV-scoped docs into their subsystem directories to reduce root clutter (59 → 43 entries). All cross-references updated.

### Moved files
- `GO_PYTHON_ALIGNMENT.md` → `bitmagnet-media/docs/04-classifier/`
- `PRUNING.md` → `bitmagnet-media/docs/03-operations/`
- `BACKFILL_PROCEDURE.md` → `bitmagnet-media/docs/03-operations/`
- `IPTV_ANALYSIS_INDEX.md` → `iptv-aggregator/docs/`
- `tests/dht_pipeline/` → `bitmagnet-media/tests/dht_pipeline/`
- `test_plan/*.csv` → `scripts/testing/test_plan/`

### Updated references
- DOCUMENTATION_INDEX.md, AGENTS.md, SCRIPTS_CATALOG.md, TESTING_RUNBOOK.md, PROJECT_OVERVIEW.md, BITMAGNET_OPERATIONS.md, bitmagnet-media/DOCUMENTATION_INDEX.md, scripts/README.md, iptv-aggregator/docs/IPTV_ANALYSIS_INDEX.md
- 6 scripts updated for `test_plan/` → `scripts/testing/test_plan/` path change
- test_runner.py stale import fixed (`bitmagnet/classifier-tools` → `classifier`)

### Phase 1 changes (also 2026-04-22)
- Deleted: MAGNETICO_SEARCH_MASTER_PLAN.md (superseded), output/ (empty), traefik/acme.json.bak, traefik/docker-compose.yml, root iptv-addon.env (stale copy)
- Moved: DATABASE_STORAGE_ANALYSIS.md → bitmagnet-media/docs/02-architecture/, BITMAGNET_PIPELINE_ROADMAP.md → bitmagnet-media/docs/03-operations/, run_protocol.sh + test_aiostreams_stream.py → scripts/testing/, branding/ → resources/branding/, systemd units → scripts/systemd/
- .gitignore: added logs/, test_runs/, output/, *.lock
...
- Documentation updates:
  ...
  - Created `bitmagnet-media/docs/04-classifier/WORKFLOW.md` - Full pipeline documentation
